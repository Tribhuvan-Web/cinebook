...
public class DataInitializer {
        ...
        @Bean
        CommandLineRunner initDatabase(MovieRepository movieRepository, MovieSlotRepository movieSlotRepository) {
                return args -> {
                        LocalDate startOfRange = LocalDate.now();
                        LocalDate endOfRange = startOfRange.plusDays(5);

                        // Create and save movies first
                        List<AvailableMovie> movies = Arrays.asList(
                                        new AvailableMovie(
                                                        1L,
                                                        "F1: The Movie",
                        ...
                        // Save all movies
                        List<AvailableMovie> savedMovies = movieRepository.saveAll(movies);
