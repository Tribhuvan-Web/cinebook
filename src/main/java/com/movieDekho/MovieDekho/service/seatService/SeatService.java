...
public class SeatService {
    ...
    public List<SeatResponse> createSeats(Long slotId, List<SeatRequest> seatRequests) {
        ...
        // OPTIMIZATION: Extract seat numbers efficiently
        List<String> requestSeatNumbers = seatRequests.stream()
                .map(SeatRequest::getSeatNumber)
                .toList();

        // OPTIMIZATION: Check for duplicates in a single query
        ...
        // OPTIMIZATION: Create seat entities in batch
        List<Seat> seats = seatRequests.stream()
                .map(request -> createSeatEntity(slot, request))
                .collect(Collectors.toList());

        // OPTIMIZATION: Save all seats in one batch operation
        List<Seat> savedSeats = seatRepository.saveAll(seats);

        // OPTIMIZATION: Update slot totals once at the end
    ...
    /**
     * OPTIMIZED: Bulk create seats with pattern with duplicate checking and batching
     */
    public List<SeatResponse> bulkCreateSeats(Long slotId, String rowStart, String rowEnd,
        ...
        // OPTIMIZATION: Check for existing seats in batch to prevent duplicates
        List<String> seatNumbers = seats.stream()
                .map(Seat::getSeatNumber)
                .toList();
        
        List<Seat> existingSeats = seatRepository.findBySlotAndSeatNumberIn(slot, seatNumbers);
        if (!existingSeats.isEmpty()) {
            List<String> duplicates = existingSeats.stream()
                    .map(Seat::getSeatNumber)
                    .toList();
        ...
        // OPTIMIZATION: Use saveAll for batch insert
        List<Seat> savedSeats = seatRepository.saveAll(seats);

        // OPTIMIZATION: Update slot totals once at the end
        updateSlotTotals(slot, seats.size(), seats.size());

        log.info("Bulk created {} seats for slot {} in optimized manner", seats.size(), slotId);
