...
public interface TemporarySeatLockRepository extends JpaRepository<TemporarySeatLock, Long> {
    ...
    List<TemporarySeatLock> findActiveLocksBySeats(@Param("seats") List<Seat> seats,
            @Param("currentTime") LocalDateTime currentTime);

    // Find active locks for specific slots
    @Query("SELECT tsl FROM TemporarySeatLock tsl WHERE tsl.slot IN :slots AND tsl.isActive = true AND tsl.expiresAt > :currentTime")
    List<TemporarySeatLock> findActiveLocksBySlots(@Param("slots") List<MovieSlot> slots,
            @Param("currentTime") LocalDateTime currentTime);
    ...
    Optional<TemporarySeatLock> findActiveLockBySeat(@Param("seat") Seat seat,
            @Param("currentTime") LocalDateTime currentTime);
    ...
    boolean isSeatLockedByOtherSession(@Param("seat") Seat seat, @Param("sessionId") String sessionId,
            @Param("currentTime") LocalDateTime currentTime);
